...
class KnowledgeBase:
    ...
    def graph(self):
        ...
        if constants_path and not self._axiom_keys:
    ...
    def add(self, entry: KBEntry) -> bool:
        ...
        if entry.level == "L0":
            return False
        if not entry.scope or not isinstance(entry.scope, dict):
            return False
        if entry.key in self._entries:
            # Update existing entry
            old = self._entries[entry.key]
            self._index.remove(entry.key, old.content, old.tags)
            self._entries[entry.key] = entry
    ...
    def query(self, query_text: str, top_k: int = 10) -> List[Tuple[KBEntry, float]]:
        """语义检索: 通过 BM25 索引查找条目。
    ...
    def search(self, query_text: str, top_k: int = 10) -> List[KBEntry]:
        """同 query,但只返回 KBEntry 列表(不含分数)。"""
        return [entry for entry, _ in self.query(query_text, top_k=top_k)]
    ...
    def mark_success(self, key: str):
        ...
        elif lv == 0 and entry.usage_count >= 3:
            if entry.confidence >= 0.6:
                entry.level = "L1"
                entry.upgrade_cooldown = 3
    ...
    @staticmethod
    def _semantically_related(text_a: str, text_b: str) -> bool:
        """判断两段文本是否语义相关(基于关键词重叠 >= 5)。"""
        def extract_keywords(text: str) -> Set[str]:
            words: Set[str] = set()
            en_words = re.findall(r"[a-zA-Z]{3,}", text.lower())
            for w in en_words:
