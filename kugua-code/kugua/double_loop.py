...
class DoubleLoopExecutor:
    ...
    def __init__(
        ...
        meta_reviewer: Optional[Any] = None,
                ...
                if gate_type == "GATE_PROPOSAL":
                    if hasattr(self.observer, "gate_proposal"):
                        result = self.observer.gate_proposal(
                            gv_before=event.gv_content_before,
                            gv_after=event.gv_content_after,
    ...
    def _blind_audit(
        ...
        优先使用 MetaReviewer (4模板×3审查者, ≥2/3通过),
        不可用时回退到内联 3 视角审查。
        """
        audit = {
            "method": "inline",
            ...
            "passed": False,
            "reviewer_count": 0,
            "meta_reviewer_result": None,
        }

        # ── 优先: MetaReviewer 深度盲审 ──
        if self.meta_reviewer is not None and llm_client is not None:
            try:
                mr_result = self.meta_reviewer.audit(
                    gv_content_before=event.gv_content_before or "",
                    gv_content_after=event.gv_content_after or "",
                    reason=event.modification_reason or "",
                    impact=f"GV: {event.gv_id}, Error: {event.error_type}",
                    error_pattern_summary=event.root_cause_summary or "",
                )
                audit["method"] = "meta_reviewer"
                audit["passed"] = mr_result.passed
                audit["overall_score"] = mr_result.approve_count / max(len(mr_result.votes), 1)
                audit["reviewer_count"] = len(mr_result.votes)
                audit["meta_reviewer_result"] = mr_result.to_dict()
